...
- **Requirement Workflow System**: Requirements follow a hierarchical assignment flow: Admin creates requirement → assigns to Team Lead (TL) → TL sees unassigned requirements in their dashboard → TL assigns to a team member (Recruiter/Talent Advisor from their team only) → Recruiter sees requirement in their dashboard. The `/api/team-leader/requirements` endpoint fetches both recruiter-assigned requirements and unassigned requirements where `teamLead` matches the TL's name. When TL assigns via `/api/team-leader/requirements/:id/assign-ta`, both `talentAdvisor` (name) and `talentAdvisorId` (ID) are set for proper ID-based lookup. Recruiters fetch their requirements via `talentAdvisorId` matching.
...
- **Admin Dashboard Data Persistence**: All admin dashboard data (Performance charts, Target Mappings, Revenue Analysis, Team Performance, Closures List) now persists in the PostgreSQL database. React Query manages data fetching with proper cache invalidation. The Performance LineChart uses the `/api/admin/monthly-performance` API endpoint for real data instead of hardcoded arrays.
- **Admin Data Reset System**: Admin dashboard includes reset functionality for Performance Data (target/revenue mappings) and Master Data (candidates/deliveries). Reset operations use confirmation dialogs and properly invalidate all related React Query caches to ensure frontend state reflects database changes.
