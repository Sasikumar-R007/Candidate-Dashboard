...
async function runMigration() {
  try {
    ...
    if (migrationFile === "add_last_login_at.sql") {
      console.log(
        "✅ Column last_login_at has been added to the employees table"
      );
      ...
      if (result.rows.length === 1) {
        ...
        console.log(
          "⚠️  Warning: Could not verify column. Please check manually."
        );
      }
    } else {
      console.log(
    ...
    if (error.code === "42703") {
      console.error(
        "   This might mean the columns already exist or there was a different error."
      );
    }
    process.exit(1);
  } finally {
