...
// 1. Color change for Zap icon
content = content.replace(
  'className="w-4 h-4 text-red-500 fill-red-500 relative z-10 animate-pulse"',
  'className="w-4 h-4 text-blue-500 fill-blue-500 relative z-10 animate-pulse"'
);

// 2. Add query and state
const queryTarget = `  const { data: applications = [], isLoading } = useJobApplications();`;
const queryReplace = `  const { data: applications = [], isLoading } = useJobApplications();
...
// 3. Add Nudges section
const targetUI = `{/* Applied Jobs Section - Pipeline Layout */}
          <div className="bg-white rounded-md p-4 shadow-sm relative border border-gray-100">`;

const replaceUI = `{/* Candidate Nudges Updates Section */}
...
// 4. Add Dialog for View More
const targetDialog = `{/* Withdraw Confirmation Dialog */}`;
const replaceDialog = `{/* Full Nudge Logs Dialog */}
...
content = content.replace(targetDialog, replaceDialog);

fs.writeFileSync('client/src/components/dashboard/tabs/my-jobs-tab.tsx', content);
console.log("Patched my-jobs-tab.tsx with Candidate Nudges UI.");
